...
class UVProjection():
	...
	@torch.no_grad()
	def render_mesh(self, image_size=1024):
		if image_size:
			size = self.renderer.rasterizer.raster_settings.image_size
			self.renderer.rasterizer.raster_settings.image_size = image_size
		tmp_mesh = self.mesh.clone()
		...
		self.renderer.shader = HardNChannelFlatShader(device=self.device, cameras=self.cameras[0], lights=lights)
		
		images = self.renderer(tmp_mesh.extend(len(self.cameras)), cameras=self.cameras, lights=lights)
		self.renderer.shader = shader
